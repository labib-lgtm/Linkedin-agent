...
asset_path back to the angle record.
...
from supabase_client import get_angle, update_angle
...
def main() -> None:
    ...
    if not args.allow_any_status:
        current = (angle.get("status") or "").strip()
    ...
    update_angle(
        args.angle_id,
        {"status": "Visual Ready", "asset_path": rel_path},
    )
