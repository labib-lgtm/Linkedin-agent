...
|---|---|---|
| **Source of truth** | **Supabase Postgres** (`SUPABASE_URL`, schema in [db/schema.sql](../db/schema.sql)) | Replaces the legacy Google Sheet. Tables: `angles`, `lead_magnet_recipients`, `audit_log`, `patterns`, `killed_topics`, `metrics`. |
| Human review surface | **Webapp** ([webapp/](../webapp/)) | Next.js + Tailwind, mobile-friendly. Kanban pipeline + angle detail + recipient log + calendar. Hosted on Lovable. |
...
| Client pattern data source | **Supabase `patterns` table** | Anonymized: vertical, ad spend, ACoS before/after, what worked, what failed. Still needed for posts beyond the 10 already drafted. |
...
| **Approval gate (GATE 1)** | **`angles` table — `status` field** | Replaces verbal pick-3-4. Agent writes angles as `Pending`; user marks `Approved`/`Killed` in the webapp at their own pace. `04_post_writer` reads `status=Approved` records. |

## Workflow chain

```
Sunday eve  → 01_niche_research → 02_creator_tracker → 03_topic_pipeline
              → sheets_push_angles → [GATE 1: user marks Approved in webapp]
Monday AM   → 04_post_writer (reads angle record) → 05_visual_asset → [GATE 2: edit + approve in chat]
...
(Tool filenames keep the `sheets_*` prefix for backwards compatibility with workflow doc commands; their implementations now read/write Supabase.)
