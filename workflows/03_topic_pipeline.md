...
- Sheet `killed_topics` tab — angles Labib previously marked `Killed` (read via `sheets_client.worksheet('killed_topics')`); don't re-suggest
- Sheet `angles` tab `Pending` rows — these ARE the backlog; consider leftover Pending angles before generating new ones
...
- `tools/sheets_push_angles.py` — pushes the markdown angles into the Sheet's `angles` tab as `Pending`
...
## Steps
1. Load all input sources. Read `killed_topics` tab + existing `Pending` angles from the Sheet so we don't regenerate ideas already on the table.
...
3. Drop any idea that overlaps a row in the Sheet's `killed_topics` tab.
...
6. Push the markdown to the Sheet: `python tools/sheets_push_angles.py --source temp/outputs/topics/YYYY-WW.md --week YYYY-WW`. Surface a one-line summary in chat: "Pushed N angles to Sheet, M skipped as duplicates. Sheet URL: …"

## Output
- Persisted markdown: `temp/outputs/topics/YYYY-WW.md` (full ranked list, archive copy)
- Live state: Sheet `angles` tab — N rows added with `status=Pending`
- Chat: a single summary line + Sheet link. No more "let me show you the ranked list and you pick" — Labib approves at his own pace inside the Sheet.

## Edge cases
- Fewer than 8 viable ideas → say so out loud, ask whether to broaden inputs or run with what we have.
- Labib kills all suggestions in the Sheet → after a week with `Killed` rows piling up, re-read `killed_topics` and ask "what kind of angle do you want instead?" before regenerating.
- Sheet API is down → the markdown is still saved. Re-run `sheets_push_angles.py` later. No data lost.

## Hand-off
[04_post_writer.md](04_post_writer.md) reads `Approved` rows from the Sheet — this workflow no longer hands off topics directly; the Sheet is the queue.
